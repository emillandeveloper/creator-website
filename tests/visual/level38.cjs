...
test("Chrome visual and interaction QA at desktop, tablet, phone and landscape sizes", { timeout: 180000 }, async (t) => {
  ...
  await op("polls", { type: "CUSTOM", title: "Where should the adventure take us next?", options: [{ label: "Explore the ancient ruins" }, { label: "Challenge the optional boss" }, { label: "Hunt for rare treasure" }] });
  ...
  const out = path.resolve("dist/phase3-visual-qa"); fs.mkdirSync(out, { recursive: true });
  ...
  fs.writeFileSync(path.join(out, "results.json"), JSON.stringify({ browser: await browser.version(), widths: [1440,1024,768,390], landscape: [844,390], errors, csp, screenshots: fs.readdirSync(out).filter((file) => file.endsWith('.png')) }, null, 2));
});
