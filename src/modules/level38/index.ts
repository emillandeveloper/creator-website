...
export function mountLevel38(app: Application, server: HttpServer, config: Level38Config) {
  ...
  const service = new Level38Service(db, (state, change) => {
  ...
  const controller = new Level38Controller(db, new Level38Auth(db, config), service);
  app.use("/level38", createLevel38Routes(controller, config));
